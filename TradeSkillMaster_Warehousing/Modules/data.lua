...
function data:getEmptyRestoreGroup(container, isGuildBank)
	...
	for i, bagid in ipairs(container) do
		for slotid = 1, TSM.util.getContainerNumSlotsSrc(bagid) do
			...
			if id and tonumber(string.match(id, "item:(%d+):")) ~= 110000 then
				if (not isGuildBank or GuildBankFrameTab1:GetText() == "Personal Bank") or not TSMAPI:IsSoulbound(bagid, slotid) --Empty should include soulbound items if personalbank 
				then
